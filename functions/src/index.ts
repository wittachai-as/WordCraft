...
type Recipe = { id: string; name: string; type?: 'start' | 'goal' | 'result' };

function slugify(text: string): string {
  return text.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
}

async function computeAndCache(dateISO: string, a: string, b: string): Promise<Recipe | null> {
  if (!dateISO || !a || !b) return null;
  ...
  const dataResp = await resp.json() as Recipe;
...
export const computeRecipe = functions.region('asia-southeast1').https.onCall(async (data, context) => {
  const { dateISO, a, b } = data as { dateISO: string; a: string; b: string };
  if (!dateISO || !a || !b) {
